...
TRACER_PINTOOL_PATH = 'BranchNet/src/tracer/obj-intel64/tracer.so'
...
def get_run_cmd(trace_path):
    pin_root = '/home2/dongxu/sde-external-9.7.0-2022-05-09-lin/pinkit'
    # assert os.path.exists(TRACER_PINTOOL_PATH)
    assert os.path.exists(pin_root + '/pin')
    ...
    return ('{pin_root}/pin'
            + ' -t {pintool_path}'
            + ' -trace_out_file {output_path} -- /bin/ls').format(
                pin_root=pin_root,
                pintool_path=TRACER_PINTOOL_PATH,
                output_path=trace_path,
            )
...
def main():
    ...
    # common.run_parallel_commands_local(get_run_cmd('/home2/dongxu/BranchNet/traces'), NUM_THREADS)
    print(get_run_cmd('/home2/dongxu/BranchNet/b'))
