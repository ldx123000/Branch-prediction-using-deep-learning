...
class AggregateBranchStats:
  ...
  def finalize_stats(self):
    ...
    unweighted_correct = self.unweighted_stats.correct
    unweighted_total = self.unweighted_stats.total
    unweighted_instructions = self.unweighted_stats.instructions
...
def update_tage_stats(tage_stats, stats_file):
    PC_COLUMN_IDX = 0
    ...
    with open(stats_file) as f:
        ...
        for cols in reader:
            ...
            if br not in tage_stats:
                tage_stats[br] = AggregateBranchStats()
            tage_stats[br].unweighted_stats.InitWithCorrect(
                correct, total)


def read_tage_stats(tage_config_name, stats_file, hard_brs_tag=None):
    tage_stats = {}
    update_tage_stats(tage_stats, stats_file)
