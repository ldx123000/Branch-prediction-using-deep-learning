  ...
  def get_dataloader(self, traces, eval=False):
    ...
    if len(dataset) > 0:
      dataloader = DataLoader(
          ...
          shuffle=not eval, num_workers=1, pin_memory=True)
    else:
      dataloader = None
    print('Debug: len(dataset): ' + str(len(dataset)))
    print('Debug: initialise dataset' + ' pc: ' + str(self.br_pc) + ' traces: ' + str(traces))
    print(dataloader)
